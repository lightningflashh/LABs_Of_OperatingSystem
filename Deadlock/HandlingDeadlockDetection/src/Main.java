...
public class Main {
    ...
    static String fileName = "testcase_3.txt";
    ...
    static void safe() {
        ...
        if (count < rows)
        {
            System.out.println("System is in Deadlock and the Deadlock process are: ");
            for (int i = 0; i < cols; i++) {
                if (!visited[i])
                    System.out.print(i + " ");
            }
    ...
    static void readFile() throws IOException {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            for (int j = 0; j < cols; j++) {
                request[i][j] = Integer.parseInt(maxLine[j]);
            }
        }
